...
use crate::capture::{self, WindowCrop};
...
// --- Global hotkey actions (sent from rdev listener in main.rs) ---
...
impl ThresholdApp {
    ...
    fn start_selection(&mut self) {
        ...
        #[cfg(target_os = "windows")]
        {
            // Parse our own HWND (stored as isize string) to u32 to match xcap's id() truncation.
            // xcap's Window::id() does `hwnd.0 as u32`; we mirror that cast here.
            // Use u32::MAX as sentinel so a failed parse can't collide with real window IDs.
            let own_id: u32 = self.window_id.as_ref()
                .and_then(|s| s.parse::<isize>().ok())
                .map(|v| v as u32)
                .unwrap_or(u32::MAX);

            // Enumerate visible windows via xcap
            ...
            if let Ok(all) = xcap::Window::all() {
                for w in all {
                    ...
                    // Filter: visible, has title, not our own window (by HWND, not title)
    ...
    fn apply_selection(&mut self, result: SelectionResult) {
        log::info!("Capturing window {} crop {}x{} at ({},{})",
            result.window_id,
            result.crop.width, result.crop.height,
            result.crop.x, result.crop.y);
        self.resize_to = Some((result.crop.width as f32, result.crop.height as f32));
        self.reposition_to = Some((result.screen_x, result.screen_y));
...
impl eframe::App for ThresholdApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Poll global hotkeys from rdev listener
        ...
        if let Some((w, h)) = self.resize_to.take() {
            ctx.send_viewport_cmd(egui::ViewportCommand::InnerSize(
                egui::vec2(w + PANEL_WIDTH, h),
            ));
        }

        // Reposition window — shift left by panel width so image area aligns with target
        if let Some((x, y)) = self.reposition_to.take() {
            let adjusted_x = x - PANEL_WIDTH as i32;
            #[cfg(target_os = "linux")]
            {
                if let Some(id) = &self.window_id {
                    match Command::new("xdotool")
                        .args(["windowmove", id, &adjusted_x.to_string(), &y.max(0).to_string()])
                        .status()
                    {
                        Ok(s) if !s.success() => log::warn!("xdotool windowmove exited: {s}"),
                        Err(e) => log::warn!("xdotool windowmove failed: {e}"),
            ...
            #[cfg(target_os = "windows")]
            {
                ctx.send_viewport_cmd(egui::ViewportCommand::OuterPosition(
                    egui::pos2(adjusted_x as f32, y.max(0) as f32),
                ));
            }
        }

        // Process pending move (Windows)
        if let Some((dx, dy)) = self.pending_move {
            if let Some(rect) = ctx.input(|i| i.viewport().outer_rect) {
                ...
                self.pending_move = None; // only consume when actually applied
            }
        }
        ...
        if self.target_window_id.is_some() && !self.is_selecting()
        ...
        // Left control panel with scroll for short windows
        egui::SidePanel::left("controls")
            ...
            .show(ctx, |ui| {
                egui::ScrollArea::vertical().show(ui, |ui| {
                    ...
                    if btn.on_hover_text(format!("Select window + area ({})", self.key_reselect_name)).clicked() {
                        self.start_selection();
                    }
                    ...
                    if ui.checkbox(&mut aot, "Top").on_hover_text(format!("Always on top ({})", self.key_toggle_top_name)).changed() {
                    ...
                    ui.label("Move");
                    ui.horizontal(|ui| {
                        if ui.button("<").clicked() { self.move_window(-MOVE_STEP, 0); }
                        if ui.button(">").clicked() { self.move_window(MOVE_STEP, 0); }
                    });
                    ui.horizontal(|ui| {
                        if ui.button("/\\").clicked() { self.move_window(0, -MOVE_STEP); }
                        if ui.button("\\/").clicked() { self.move_window(0, MOVE_STEP); }
                    });
    ...
    fn render_capture_display(&self, ui: &mut egui::Ui) {
        if let Some(tex) = &self.texture {
            ...
            ui.centered_and_justified(|ui| {
                #[cfg(target_os = "linux")]
                ui.label("Step 1: Click on the window to capture\nStep 2: Draw a rectangle on the area you want");
                #[cfg(target_os = "windows")]
            ...
            ui.centered_and_justified(|ui| {
                ui.label(format!("Press {} to select a window and area.", self.key_reselect_name));
            });
        }
    }
}
...
impl ThresholdApp {
    ...
    fn render_window_picker(&mut self, ctx: &egui::Context, ui: &mut egui::Ui) {
        ...
        if let Some(wid) = picked {
            // Find window info
            if let Some((_id, _title, wx, wy, ww, wh)) = windows.iter().find(|(id, ..)| *id == wid) {
                ...
                if let Ok(all) = xcap::Window::all() {
                    if let Some(win) = all.into_iter().find(|w| w.id().unwrap_or(0) == wid) {
                        if let Ok(img) = win.capture_image() {
                            ...
                            let tex = ctx.load_texture("preview", color_img, egui::TextureOptions::NEAREST);
    ...
    fn render_region_drawer(&mut self, _ctx: &egui::Context, ui: &mut egui::Ui) {
        ...
        // We need to extract fields from the enum to avoid borrow issues
        let (window_id, win_x, win_y, tex_id, real_w, real_h, logical_w, _logical_h) = {
            let WinSelection::DrawingRegion {
                window_id, win_x, win_y, preview, real_w, real_h,
                logical_w, logical_h, ..
            } = &self.win_selection else {
                return;
            };
            (*window_id, *win_x, *win_y, preview.id(), *real_w, *real_h, *logical_w, *logical_h)
        };
        ...
        let drag_start_pos = if let WinSelection::DrawingRegion { drag_start, .. } = &self.win_selection {
            *drag_start
        } else {
            None
        };
        ...
        if response.drag_stopped() {
            let drag_rect = if let WinSelection::DrawingRegion { drag_rect, .. } = &self.win_selection {
                *drag_rect
            } else {
                None
            };
            ...
            if let Some(rect) = drag_rect {
                if rect.width() > 5.0 && rect.height() > 5.0 {
                    ...
                    // Convert physical-pixel crop offset to logical pixels for screen positioning.
                    // win_x/win_y are logical (from xcap's rcClient), crop_x/crop_y are physical
                    // (relative to the captured image which is scaled by DPI).
                    ...
                    // apply_selection uses physical crop dims; we need logical for the window.
                    self.resize_to = Some((
                        (crop_w as f32 / dpi_scale).max(1.0),
                        (crop_h as f32 / dpi_scale).max(1.0),
                    ));
                } else {
                    // Too small — return to window picker
                    self.win_selection = WinSelection::Idle;
                    self.start_selection();
                }
            } else {
                // Click with no drag — return to window picker
                self.win_selection = WinSelection::Idle;
                self.start_selection();
            }
        }
...
fn do_selection_linux() -> anyhow::Result<SelectionResult> {
    ...
    let output = Command::new("xdotool").arg("selectwindow").output()
    ...
    let window_id: u32 = wid_str.parse()
